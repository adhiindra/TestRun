...
const TodoData = types.model({
    ...
    addTodo: (title: string, date: string, status: string) => {
        let tempTodos: any = self.todos || []
        tempTodos.push({ id: Math.random(), title: title, date: date, status: status})
    ...
    updateStatusTodo: (id: number, newStatus: string) => {
        let tempTodos: any = self.todos || []
        const index = tempTodos.map(function (x) { return x.id; }).indexOf(id);
